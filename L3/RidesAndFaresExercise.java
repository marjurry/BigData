...
public class RidesAndFaresExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends RichCoFlatMapFunction<TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		...
		// State to store the ride if it arrives before the fare
		private transient ValueState<TaxiRide> rideState;
		// State to store the fare if it arrives before the ride
		...
		public void flatMap1(TaxiRide ride, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			...
			if (fare != null) {
				// Fare arrived earlier, emit the pair and clear the fare state
				fareState.clear();
				out.collect(new Tuple2<>(ride, fare));
			} else {
				// Fare hasn't arrived yet, store the ride
		...
		public void flatMap2(TaxiFare fare, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			...
			if (ride != null) {
				// Ride arrived earlier, emit the pair and clear the ride state
				rideState.clear();
				out.collect(new Tuple2<>(ride, fare));
			} else {
				// Ride hasn't arrived yet, store the fare
